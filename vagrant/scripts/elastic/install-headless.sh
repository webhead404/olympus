...
[Service]
ExecStart=nohup /opt/toolbox/headless/bin --sessionToken $OPERATOR_SESSION_KEY --accountEmail $OPERATOR_LOGIN_EMAIL --accountToken $OPERATOR_LOGIN_TOKEN >/opt/headless/headless.log 2>&1  &
User=vagrant
Group=vagrant
Restart=on-failure
...
#cd "$(mktemp -d)"
#curl --silent -LJ https://go.dev/dl/go1.19.2.linux-amd64.tar.gz | tar xzf -

# remove line below when Redops is fixed
#rm /usr/bin/go
#mv go /usr/local