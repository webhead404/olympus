# Purpose: Joins a Windows host to the thremulator.local.local domain which was created with "create-domain.ps1".
...
If (($hostname -eq "wef") -or ($hostname -eq "exchange")) {
  ...
  While ($tries -lt 3) {
    Try {
      $tries += 1
      Add-Computer -DomainName "thremulator.local" -credential $DomainCred -OUPath "ou=Servers,dc=thremulator,dc=local" -PassThru -ErrorAction Stop
      Break
    } Catch {
      $tries += 1
