...
# Stage license file
Copy-Item "$resource_dir\aurora-lite.lic" "$install_dir\aurora-agent-lite-win-pack"

cd $install_dir\aurora-agent-lite-win-pack
.\aurora-agent.exe --install --dashboard -c agent-config-standard.yml --json --logfile aurora_alerts.json